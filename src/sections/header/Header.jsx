...
const Menu = ({menuitem}) =>{
    ...
    return(
        <>
        {
            menuitem.type === "link"
            && 
            <li className='menu hover:font-bold w-full lg:w-auto hover:text-yellow-500 lg:hover:text-primary transition-all ease-out duration-200'><a href={menuitem.path}>{menuitem.name}</a></li>
        }
        {
                        ...
                        (
                            <div className="dropdown-items static lg:absolute left-1 lg:min-w-[8rem] bg-transparent lg:bg-cyan-50 p-2 mt-4 flex flex-col gap-4 shadow-sm">
                                ...
                                menuitem.dropdownitems.map((item)=>
                                    <li className='menu hover:font-bold w-full lg:w-auto hover:text-yellow-500 lg:hover:text-primary transition-all ease-out duration-200'><a href={item.path}>{item.name}</a></li>
                                )
                                }
                            </div>
                        )
        ...
        <div className={`container p-2 h-[80px] lg:min-h-[10vh] relative lg:static flex`}>
            <div className="row w-full flex gap-8 items-center">
                ...
                <div className="col grow flex justify-end items-center">
                    ...
                    <div className={`nav-wrapper absolute top-[80px] ${showMobileMenu ? `right-0`: `-right-full`} w-full lg:w-auto py-8 lg:p-2 bg-dark-hard text-white text-lg lg:bg-inherit lg:text-inherit lg:static transition-all ease-in duration-500 lg:transition-none`}>
